...
func WriteMessage(buf *bytes.Buffer, msg Message, pver uint32) (int, error) {
	fmt.Printf("📨 [WriteMessage] Adding Lightning message header:\n")
	fmt.Printf("   Message Type: %s (0x%04x)\n", msg.MsgType(), uint16(msg.MsgType()))
	...
	totalBytes := buf.Len() - oldByteSize
	fmt.Printf("📨 [WriteMessage] Complete Lightning message:\n")
	fmt.Printf("   Total size: %d bytes (header: %d + payload: %d)\n", totalBytes, msgTypeBytes, lenp)
	fmt.Printf("   Complete wire format: %x\n", buf.Bytes()[oldByteSize:])
	fmt.Printf("   🔐 Sending to Brontide encryption...\n\n")
