...
func (p *paymentLifecycle) sendAttempt(
	...
	// 🔧 LOG: Show individual components BEFORE they are joined
	log.Infof("🔧 [PAYMENT] Building UpdateAddHTLC with separate components:")
	log.Infof("   💰 Amount: %d msat (%.8f BTC)", rt.FirstHopAmount.Val.Int(), float64(rt.FirstHopAmount.Val.Int())/1e11)
	log.Infof("   ⏰ Expiry: %d blocks", rt.TotalTimeLock)
	log.Infof("   🔐 PaymentHash: %x", *attempt.Hash)
	log.Infof("   📦 CustomRecords: %v", rt.FirstHopWireCustomRecords)
	...
	log.Infof("   🧅 OnionBlob generated: %d bytes", len(onionBlob))
	if len(onionBlob) > 32 {
		log.Infof("   🧅 OnionBlob preview: %x...", onionBlob[:32])
	} else {
		log.Infof("   🧅 OnionBlob: %x", onionBlob)
	}
