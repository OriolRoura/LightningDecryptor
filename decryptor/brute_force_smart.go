...
func (bd *BrontideDecryptor) analyzeLightningMessage(plaintext []byte, frameNumber string) {
	...
	// Detailed message-specific analysis using consolidated dispatcher
	analyzer := bd.getMessageAnalyzer(msgType)
	if analyzer != nil {
		analyzer(plaintext)
	}
...
// printHexDump prints a compact hex dump (limited to 256 bytes for performance)
...
// analyzeFeatureBits analyzes Lightning feature bits (simplified)
func (bd *BrontideDecryptor) analyzeFeatureBits(features []byte) {
	knownFeatures := map[int]string{
		0: "data_loss_protect", 6: "gossip_queries", 8: "var_onion",
		12: "static_remotekey", 14: "payment_secret", 16: "basic_mpp",
		20: "anchor_outputs", 30: "onion_messages", 32: "channel_type",
	}
	...
	for i := len(features) - 1; i >= 0; i-- {
		for bit := 0; bit < 8; bit++ {
			if features[i]&(1<<bit) != 0 {
				...
				if name, ok := knownFeatures[bitPos]; ok {
	...
	if len(active) > 0 {
		dualPrintf("   Features: %v\n", active)
	}
}

// Consolidated message analysis - replaces multiple stub functions
var messageTypeMap = map[uint16]string{
	17: "error", 18: "ping", 19: "pong", 32: "open_channel", 33: "accept_channel",
	34: "funding_created", 35: "funding_signed", 36: "funding_locked", 38: "shutdown",
	39: "closing_signed", 131: "update_fail_htlc", 134: "update_fee",
	135: "update_fail_malformed_htlc", 256: "channel_announcement",
	257: "node_announcement", 258: "channel_update", 259: "announce_signatures",
}
...
func (bd *BrontideDecryptor) analyzePongMessage(data []byte) {
	...
	dualPrintf("\n🏓 PONG: %d ignored bytes\n", binary.BigEndian.Uint16(data[2:4]))
}

// All remaining message types use generic analysis
func (bd *BrontideDecryptor) getMessageAnalyzer(msgType uint16) func([]byte) {
	switch msgType {
	case 16:
		return bd.analyzeInitMessage
	case 17:
		return bd.analyzeErrorMessage
	case 18:
		return bd.analyzePingMessage
	case 19:
		return bd.analyzePongMessage
	case 128:
		return bd.analyzeUpdateAddHTLCMessage
	case 130:
		return bd.analyzeUpdateFulfillHTLCMessage
	case 132:
		return bd.analyzeCommitmentSignedMessage
	case 133:
		return bd.analyzeRevokeAndAckMessage
	case 136:
		return bd.analyzeChannelReestablishMessage
	default:
		return func(data []byte) { bd.analyzeUnknownMessage(data, msgType) }
	}
}
